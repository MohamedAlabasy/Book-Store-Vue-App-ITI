            ...
            <nav class="navbar navbar-dark bg-dark align-items-baseline">
                <div class="container-fluid">
                    <a href="#" @click.prevent="isWatchListVisible=false" class="navbar-brand fs-3"
                        style="color: wite; text-decoration: none;">All Books</a>
                    <div class="d-flex align-items-baseline text-light">
                        <!-- <p>[ {{booksListCount() }} ] {{booksListCount() == 1 ? 'Book' : 'Books'}} with total price
                            :
                            [ {{handleCurrency(getTotalPrice()) }} ]</p> -->
                        <button class="btn btn-success mx-2" @click="isWatchListVisible=true">Show List</button>
                    </div>
            ...
            <div class="container-fluid mt-3" v-if="isWatchListVisible==false">
                <div class="row d-flex justify-content-around text-center">
                    ...
                    <div v-for="book in books" :key="book.id" style="width: 15rem; margin: 0.2rem;"
                        :class="['card p-0 mb-3 border border-1',book.pages<50?'border-danger':'border-success']">
                        <img :src="book.image" style="max-height: 350px;" class="card-img-top">
                        <div class="card-footer fs-6 font-weight-bold border border-bottom-2 text-center">{{book.name}}
                        </div>
                                ...
                                <tr v-for="item in booksList">
                                    ...
                                    <td>
                                        <button class="btn btn-danger" type="button"
                                            @click="removeFromList(item)">Remove</button>
                                    </td>
                                </tr>
                            </tbody>
                            <tfoot>
